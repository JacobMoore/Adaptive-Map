...
public class AppCanvas extends JPanel {
	...
	private TranslucentButton pageButton, zoomInButton, zoomOutButton, startButton;
	...
	/**
	 * Default constructor.
	 * @param vSpaceManager The virtual space to use.
	 * @param appFrame The frame to use.
	 * @param context The applet's context.
	 */
	...
	/**
	 * Gets the number of nodes in a chapter;
	 * used to set the chapter nodes' color fill.
	 * @param chapterTitle
	...
	private void populateCanvas() {
        ...
        for (Entry<String, ChapterProperties> chapterProperty : XmlParser
            ...
            defaultNodes.put(chapterProperty.getKey(), chapterProperty.getValue().getDefaultNode());
        }
	...
	private void parseChapterLinks() {
	    for ( String chapter : nodeMap.getChapters() ) {
	        ...
	        {
	        	if ( getChapterNode(l.getFromNode().getNodeChapter()).equals(currentChapter) 
	        			&& !l.getFromNode().equals(currentChapter) && 
	        			!getChapterNode(l.getToNode().getNodeChapter()).equals(currentChapter))
	        	{
	        		Node targetChapter =  getChapterNode(l.getToNode().getNodeChapter());
   ...
   private void addTools()
   {
        ...
        toolsPane.add(zoomInButton, (Integer)(JLayeredPane.DEFAULT_LAYER+50));
        ...
        toolsPane.add(zoomOutButton, (Integer)(JLayeredPane.DEFAULT_LAYER+50));
        ...
        toolsPane.add(optionsButton, (Integer)(JLayeredPane.DEFAULT_LAYER+50));
        ...
        backButton.addActionListener(
                new ActionListener() {
                    ...
                    public void actionPerformed( ActionEvent e ) {
    			        ...
    			        vSpaceManager.getActiveCamera().setAltitude(undo.cameraHeight, false);
    			        activeCamera.moveTo(undo.node.getGlyph().vx, undo.node.getGlyph().vy);
                    }
                });
   ...
   private void initializeOptionsDialog()
   {
       ...
       resetPosButton.addActionListener(
       		new ActionListener() {
       			public void actionPerformed(ActionEvent e){
       				if ( isSelectedAChapterNode() )
       				{
       ...
       dialogPanel.setLayout(new javax.swing.BoxLayout(dialogPanel, javax.swing.BoxLayout.Y_AXIS));
   ...
   /**
    * Adds a node to the undo list, removing the oldest node if 10 nodes
    * are already stored.
    * @param n
    * 		The node to add.
    */
    ...
    */
    private class SearchBarListener implements KeyListener
    {
        ...
        public void keyPressed( KeyEvent arg0 )
        {
            ...
            {
                ArrayList<Node> searchResultNodes = searchForNode( searchBar.getText() );
                ...
                else if ( searchResultNodes.size() > 1 )
                {
                    ...
                    for ( int i = 0; i < searchResultNodes.size(); i++ ) {
        				String[] row = { searchResultNodes.get( i ).getNodeTitle(), 
        						searchResultNodes.get( i ).getNodeChapter() };
        				dataModel.addRow( row );
                    }
                	JTable searchTable = new JTable(dataModel);
                	searchTable.setDefaultRenderer(searchTable.getColumnClass(0), new ChapterColorRenderer());
                	searchTable.setDefaultEditor(searchTable.getColumnClass(0), null);
                	...
                	selectedNode.unhighlightLinks(selectedNode);
                	addNodeToBackList(selectedNode);
                   	selectedNode = searchResultNodes.get(searchTable.getSelectedRow());
                   	if ( !medViewRadioButton.isSelected() )
                   	{
                   		prevSelectedNode = selectedNode;
                   		medViewRadioButton.setSelected( true );
    ...
    private class ChapterColorRenderer extends DefaultTableCellRenderer {
		...
		public ChapterColorRenderer() { super(); }
    ...
    public void showSubNodeList()
    {
    	...
    	for ( Node n : selectedNode.getSubNodeList()) {
        	...
        	for ( Link l : selectedNode.getNodeLinks() ) {
        		if ( l.getLinkType().equals(type)) {
        			...
        			if ( !targetNode.getNodeChapter().equals(selectedNode.getNodeChapter())
    	...
    	selectionTable.setDefaultRenderer(selectionTable.getColumnClass(0), new ChapterColorRenderer());
     ...
     * @param e The event that triggered this state change.
     * @param buttonIndex The button that was pressed 
     * 		( 1 for the Chapter button, 2 for the overview button).
     */
    ...
    private void switchToMidLevelView()
    {
        ...
        if ( prevSelectedNode == null || !prevSelectedNode.getNodeChapter()
        		.equals( selectedNode.getNodeChapter() ) )
        {
        	selectedNode = nodeMap.getChapterNodes(selectedNode.getNodeChapter()).get(0);
            for (Node n : nodeMap.getChapterNodes(selectedNode.getNodeChapter()))
    		{
            	if (n.getNodeTitle().equals(defaultNodes.get(selectedNode.getNodeChapter())))
            	{
            		selectedNode = n;
            		break;
            	}
    	...
    	selectedNode.moveAbsolute(getChapterNode(selectedNode.getNodeChapter())
    			.getCenterPoint().x, getChapterNode(selectedNode.getNodeChapter())
    			.getCenterPoint().y);
        showSelectedChapter();
     ...
     * @return
     *      If a zoom level switch occurred.
     */
    ...
    public void showSelectedChapter() {
        	...
        	for ( Node n : selectedNode.getSubNodeList()) {
	        	...
	        	for ( Link l : selectedNode.getNodeLinks() ) {
	        		if ( l.getLinkType().equals(type)) {
	        			...
	        			targetNode.showView(ViewType.HIDDEN, selectedNode, false);
	        		}
	        	}
        ...
        Map<Integer, Point> coordMap = nodeMap.setNodeCoordsFromFile
        		(nodeMap.getChapterNodes(selectedNode.getNodeChapter()), selectedNode);;
    	if (coordMap != null)
    	{
    		// Use GraphViz data
    		...
    		ArrayList<Point> firstLevelPoints = 
    				NodeMap.setLinkedCoordsForGraphViz(firstLevelNodes, yValues, xValues, centerIndex, selectedNode);
    	...
    	{
    		coordMap = NodeMap.
    	    		setNodeCoords(chapterNodes, selectedNode, firstLevelNodes);
        ...
        Color.RGBtoHSB(background.getRed(), background.getGreen(), background.getBlue(), hsbColor);
        hsbColor[1] = hsbColor[1] * 0.4f;
        hsbColor[2] = Math.min(1.0f, hsbColor[2] * 1.1f);
        background = new Color(Color.HSBtoRGB(hsbColor[0], hsbColor[1], hsbColor[2]));
    ...
    /**
     * Represents an item in the undo list, with selected
     * node and zoom level information.
     * @author Michel
     */
