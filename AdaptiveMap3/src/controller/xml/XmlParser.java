...
import textbook.Chapter;
import textbook.ChapterNode;
...
public class XmlParser {
	 ...
	 */
	public static Map<String, ChapterProperties> parseChapterProperties(Element documentElement) {
		NodeList nodeList = getListOfAll(Tag.CHAPTER_TYPE, documentElement);
        ...
        public static Chapter[] parseChapterNodes(Element documentElement) {
            Map<String, ChapterProperties> parseChapterProperties = parseChapterProperties(documentElement);
            Chapter[] chapters = new Chapter[parseChapterProperties.size()];
            ChapterNode[] chapterNodes = new ChapterNode[chapters.length];
            int i = 0;
            for(Entry<String, ChapterProperties> entry: parseChapterProperties.entrySet()) {
                String title = entry.getKey();
                String description = entry.getValue().getDescription();
                chapters[i] = new Chapter(title, description);
                
                int x = entry.getValue().getChapterXPos();
                int y = entry.getValue().getChapterYPos();
                chapterNodes[i] = new ChapterNode(chapters[i], x, y);
	 ...
	 */
	public static Map<String, LinkProperties> parseLinkProperties(Element documentElement) {
		NodeList nodeList = getListOfAll(Tag.LINK_TYPE, documentElement);
	 ...
	 */
	public static void parseNodeInformation(NodeMap map, int fontSize, Element documentElement) {
		NodeList nodeList = getListOfAll(Tag.NODE, documentElement);
		if (nodeList == null) {
			System.err.println("Could not import node list");
			return;
		}
	 ...
	 */
	public static List<Link> parseNodeLinks(List<Node> nodesToLink, Element documentElement) {
	...
	private static NodeList getListOfAll(Tag tag, Element documentElement) {
	...
	public static Element getNodeDocument(final File file) {
