    ...
    extends VSegment
{
    ...
    public Link(Node fromNode, Node toNode, String linkType, int aSize, boolean showText)
    {
        ...
        linkText.setFont(new Font("Arial", Font.BOLD, Configuration.LINK_FONT_SIZE));
        ...
        selectionRectangle.setTranslucencyValue(0);
    ...
    public void draw(Graphics2D g,int vW,int vH,int i,Stroke stdS,AffineTransform stdT, int dx, int dy){
    	...
    	{
        	double altitude = VirtualSpaceManager.INSTANCE.getActiveCamera().getLocation().getAltitude();
	    	float minHeight = AppCanvas.ZOOM_CHAPTER_HEIGHT;
	    	float maxHeight = AppCanvas.ZOOM_OVERVIEW_MAX;
	    	double adjustedWeight = linkWeight / Math.abs((altitude - minHeight) / (maxHeight-minHeight));
    ...
    public void refresh()
    {
        ...
        Point linkCenter = getPointAlongLink(0.45f);
        FontMetrics fontMetrics = new JPanel().getFontMetrics(linkText.getFont());
        linkText.vx = linkCenter.x - (long)fontMetrics.stringWidth(linkText.getText())/2;
        //vtext origin is at bottom left of first line
        linkText.vy = linkCenter.y + (long)fontMetrics.getHeight()/2 - linkText.getFont().getSize(); 
        virtualSpace.onTop(linkText);
        ...
        if ( linkText.isVisible()) {
	        selectionRectangle.setWidth(linkText.getFont().getStringBounds(linkText.getText(), null).getWidth()/2+6);
	        selectionRectangle.setHeight(linkText.getFont().getStringBounds(linkText.getText(), null).getWidth()/2+6);
        }
        else {
     ...
     */
    private Point2D.Double[] createTrianglePoints(Node startNode, Node endNode)
    {
        int arrowHeight = arrowSize;
        int arrowWidth = arrowSize/2;
        Point2D.Double[] vertices = new Point2D.Double[3];
        ...
        if (diffX == 0)
        {
            ...
            if (diffY > 0)
            {
                ...
                vertices[0] = new Point2D.Double(endX, endY - nodeHeight / 2);
                vertices[1] =
                    new Point2D.Double(endX - arrowWidth, endY - nodeHeight / 2
                        - arrowHeight);
                vertices[2] =
                    new Point2D.Double(endX + arrowWidth, endY - nodeHeight / 2
                        - arrowHeight);
        ...
        else if (diffY == 0)
        {
            ...
            if (diffX > 0)
            {
                ...
                vertices[0] = new Point2D.Double(endX - nodeWidth / 2, endY);
                vertices[1] =
                    new Point2D.Double(endX - nodeWidth / 2 - arrowHeight, endY
                        + arrowWidth);
                vertices[2] =
                    new Point2D.Double(endX - nodeWidth / 2 - arrowHeight, endY
                        - arrowWidth);
        ...
        else
        {
            ...
            vertices[0] = new Point2D.Double(endX, endY);
            vertices[1] = new Point2D.Double(newX1, newY1);
            vertices[2] = new Point2D.Double(newX2, newY2);
            return vertices;
        }
    }
        ...
        extends VPolygon
    {
         ...
         */
        public EndTriangle(Point2D.Double[] v, int z, Color c)
        {
            super(v, z, c);
        }
