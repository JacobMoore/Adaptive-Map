...
public class Node {
	 ...
	 * 
	 * Depth is different from distance in that, if two nodes are on the same
	 * level but are not directly linked, they will have the same depth, but
	 * will have some positive distance from each other.
	 * 
	 * @param fromNode
	...
	public static int findDistanceFrom(Node fromNode, Node toNode) {
		if (fromNode.getNodeTitle().equals("Dry Friction")
				&& toNode.getNodeTitle().equals("Rolling Resistance"))
			System.out.print("");
	 ...
	 *            The nodes that have already been traversed
	 * @return The shortest distance between the nodes
	 */
	private static int findDistanceFrom(Node fromNode, Node toNode,
			...
			if (link.contains(fromNode)) {
				...
				if (tempFromNode.equals(fromNode)) {
					...
					if (tempFromNode.equals(toNode)) {
						...
						int tempDepth = 1 + findDistanceFrom(tempFromNode,
								toNode, traversedNodes);
						if (Math.abs(shortestDepth) > Math.abs(tempDepth)) {
							shortestDepth = tempDepth;
						}
	...
	public static List<Node> getFirstLevelNodes(Node selectedNode) {
		...
		for (Link firstNodeLink : selectedNode.getNodeLinks()) {
			Node firstLevelNode = firstNodeLink.getFromNode() == selectedNode ? firstNodeLink
					.getToNode() : firstNodeLink.getFromNode();
			if (!firstLevelNode.getNodeChapter().equals(
					selectedNode.getNodeChapter())
					&& (selectedNode.multiNodes == null || !selectedNode.multiNodes
							.contains(firstLevelNode)))
	 ...
	 * @param arrowSize
	 *            The size of the links arrows.
	 * @param isChapter
	 *            If this link is between overview nodes.
	 */
	public static void link(Node node1, Node node2, String linkType,
			int arrowSize, boolean isChapter) {
		Link link = null;
	...
	/**
	 * Constructor, used for regular node
	 * 
	 * @param nodeTitle
	 ...
	 * @param nodeChapter
	 *            The chapter that the node is in.
	 * @param nodeKeywords
	 *            The keywords of the node.
	 * @param color
	 *            The color of the node, generally set by the node's chapter.
	 * @param fontSize
	 *            The initial size of the node's font.
	 */
	public Node(String nodeTitle, String nodeDescription, String nodeChapter,
		...
		this.nodeTitle.setFont(new Font("Arial", Font.BOLD, fontSize));
		setNodeDescription(nodeDescription);
		this.nodeDescription.setFont(new Font("Arial", Font.PLAIN,
				fontSize));
	 ...
	 * @param nodeChapter
	 *            The chapter the node is in.
	 * @param nodeKeywords
	 *            The keywords of the node.
	 * @param color
	 *            The color of the node, generally set by the node's chapter.
	 * @param titleFontSize
	 *            The font size for this node's title.
	 * @param descriptionFontSize
	 *            The font size for this node's description.
	 * @param gradientAdjust
	 *            The positioning of the gradient color of the node.
	 */
	public Node(String nodeTitle, String nodeDescription, String nodeChapter,
			String nodeKeywords, Color color, int titleFontSize,
			int descriptionFontSize, float gradientAdjust) {
		this(nodeTitle, nodeDescription, nodeChapter, nodeKeywords, color,
				titleFontSize);/*
		nodeRectangle = new VRoundRect(0.0, 0.0, 1, 700.0, 200.0, Color.white, Color.BLACK,
				1f, 15, 15, gradientAdjust);*/
                nodeRectangle = new VRoundRect(0.0, 0.0, 15, 700, 200, Color.white, Color.black, 15, 15);
		nodeRectangle.setOwner(Node.class);
		nodeRectangle.setType("Node");
		this.nodeTitle.setFont(VText.getMainFont().deriveFont(
				titleFontSize * 1.0f));
		this.nodeDescription.setFont(VText.getMainFont().deriveFont(
				descriptionFontSize * 1.0f));
	...
	/**
	 * Return's the average translucency of the node's rectangle, title and
	 * description.
	 * 
	 ...
	 */
	public double getNodeTitleWidth() {
            FontMetrics fontMetrics = new JPanel().getFontMetrics(nodeTitle.getFont());
            return fontMetrics.stringWidth(nodeTitle.getText());
	}
	 ...
	 */
	public double getNodeDescriptionWidth() {
            FontMetrics fontMetrics = new JPanel().getFontMetrics(nodeDescription.getFont());
            return fontMetrics.stringWidth(nodeDescription.getText());
	}
	 ...
	 */
	public double getNodeTitleHeight() {
            FontMetrics fontMetrics = new JPanel().getFontMetrics(nodeTitle.getFont());
	 ...
	 */
	public double getNodeDescriptionHeight() {
            FontMetrics fontMetrics = new JPanel().getFontMetrics(nodeDescription.getFont());
            return fontMetrics.getHeight();
	}
	...
	public void moveToFixedPos() {
            Animation nodeTranslation = VirtualSpaceManager.INSTANCE
				...
				.getAnimationFactory()
				.createGlyphTranslation(1000, nodeRectangle,
						new Point2D.Double(fixedXPos, fixedYPos), false,
						IdentityInterpolator.getInstance(), null);
		VirtualSpaceManager.INSTANCE.getAnimationManager().startAnimation(
				nodeTranslation, true);
		realignNodeText();
	 ...
	 * text and rectangle are not drawn in the same way Also set's the positions
	 * of the sub-rectangles, if there are any.
	 * 
	 */
	public void realignNodeText() {
		double titleWidth = getNodeTitleWidth();
		double titleHeight = getNodeTitleHeight();
		int NODE_PADDING = Configuration.NODE_PADDING;

		// Center the node description text
		switch (nodeView) {
		case FULL_DESCRIPTION:
			double descriptionWidth = getNodeDescriptionWidth();
			double descriptionHeight = getNodeDescriptionHeight();
			double nodeAdjustedWidth = Math.max(
					Math.max(titleWidth, descriptionWidth),
					Configuration.MIN_NODE_WIDTH) + NODE_PADDING;
			double nodeAdjustedHeight = titleHeight + descriptionHeight
					+ NODE_PADDING;
			...
			nodeTitle.moveTo(nodeRectangle.vx - titleWidth / 2 + NODE_PADDING,
		...
		case TITLE_ONLY:
			...
			nodeRectangle.setHeight(nodeAdjustedHeight / 2 + NODE_PADDING * 2);
			nodeTitle.moveTo(nodeRectangle.vx - titleWidth / 2 + NODE_PADDING,
					nodeRectangle.vy + nodeRectangle.getHeight()
							- (long) nodeTitle.getFont().getSize2D()
							- NODE_PADDING);
			refreshLinks();
			break;
		...
		if (subNodeRectangles != null) {
			...
			for (VRectangle r : subNodeRectangles) {
				...
				r.moveTo(nodeRectangle.vx, nodeRectangle.vy - nodeRectangle.vh
						+ num * (nodeDescription.getFont().getSize() + 2)
						+ (num - 1) * 2);
				num++;
			}
		}
	 ...
	 * 
	 * @param selectedNode
	 *            The node currently selected, used to set transparency.
	 * @param visibility
	...
	private void setLinksVisibility(final Node selectedNode, boolean visibility) {
		for (Link link : getNodeLinks()) {
			...
			Node linkedNode = fromNode.equals(this) ? link.getToNode()
					: fromNode;
			if (visibility == false
	...
	public void setNodeDescription(String nodeDescription) {
		if (this.nodeDescription != null) {
			virtualSpace.removeGlyph(this.nodeDescription);
                ...
                /*
		this.nodeDescription = new NodeText((double)nodeRectangle.vx, (double) nodeRectangle.vy,
				1, Configuration.NODE_DESCRIPTION_COLOR, nodeDescription);*/
                this.nodeDescription = new NodeText((long)nodeRectangle.getWidth(), (long)nodeRectangle.getHeight(), 1, Configuration.NODE_DESCRIPTION_COLOR, nodeDescription);
	}
	 ...
	 *            nodeKeywords to set
	 */
	public void setNodeKeywords(String keywords) {
	...
	public void setNodeTitle(String nodeTitle) {
		...
		this.nodeTitle = new NodeText((long)nodeRectangle.getWidth(), (long)nodeRectangle.getHeight(), 1,
	 ...
	 * @param selectedNode
	 *            The node currently selected, used to determine gradient fill
	 *            and translucency.
	 * @param animate
	 *            if the transition should be animated
	 */
	private void setNodeTranslucency(final long alpha, final Node selectedNode,
			boolean animate) {
		if (animate) {
	 ...
	 */
	public final void showView(ViewType viewType, Node selectedNode,
			boolean animate) {
		nodeView = viewType;
	 ...
	 */
	public final void showView(ViewType viewType, Node selectedNode,
			boolean animate, int depth) {
		if (Math.abs(depth) == 0)
			nodeRectangle.setTranslucencyValue(1.0f);
		else if (Math.abs(depth) == 1)
			nodeRectangle.setTranslucencyValue(0.7f);
		else if (Math.abs(depth) == 2)
			nodeRectangle.setTranslucencyValue(0.3f);
		else
			nodeRectangle.setTranslucencyValue(0.1f);
	...
	public void printDebugInfo() {
		System.out.println(nodeRectangle.getTranslucencyValue());
	}

	/**
	 * @param newSize
	 *            The new size of this node's font.
	 */
	public void resizeFont(int newSize) {
		this.nodeTitle.setFont(nodeTitle.getFont().deriveFont(
				newSize * 1.0f));
		this.nodeDescription.setFont(nodeDescription.getFont()
	 ...
	 * 
	 * @param titleSize
	 *            The new size of this node's title font.
	 * @param descriptionSize
	 *            The new size of this node's description font.
	 */
	...
	/**
	 * Goes through all links and creates a list of subnodes that represent each
	 * type of link with 3 or more occurrences.
	 */
	public void initializeMultiNodeList() {
		...
		for (Link l : getNodeLinks()) {
			if (linkOccurrences.containsKey(l.getLinkType())
					&& !l.getToNode().getNodeChapter()
							.equals(l.getFromNode().getNodeChapter()))
				linkOccurrences.put(l.getLinkType(),
						linkOccurrences.get(l.getLinkType()) + 1);
		}
		...
		for (Entry<String, Integer> entry : linkOccurrences.entrySet()) {
			Node newNode = new Node(entry.getValue() + " " + entry.getKey()
					+ " Nodes in Other Chapters", "temp", nodeChapter,
					nodeKeywords, Color.white, nodeTitle.getFont().getSize());
			...
			for (Link l : getNodeLinks()) {
				if (l.getLinkType().equals(entry.getKey())
					...
					if (l.getFromNode().getGlyph().equals(this.getGlyph())) {
						...
						Color chapterColor = l.getToNode()
								.getNodeChapterColor();
						newNode.subNodes.add(l.getToNode());
						VRectangle rect = new VRectangle(nodeRectangle.vx
								- nodeRectangle.vw, nodeRectangle.vy
								- nodeRectangle.vh, 2, nodeRectangle.vw,
								nodeRectangle.vh, chapterColor, chapterColor,
								0.5f);
						rect.setType("MultiNodeRectangle");
						newNode.subNodeRectangles.add(rect);
					} else {
						names += l.getFromNode().getNodeTitle().toUpperCase();
						Color chapterColor = l.getFromNode()
								.getNodeChapterColor();
			...
			newNode.nodeDescription.setText(Link
					.getLinkProperty(entry.getKey()).getDescription() + names);
	...
	/**
	 * Defines a chapter's properties (color and description). !NOTE!: This code
	 * is a duplicate of AdaptiveMapGUI's ChapterProperties! Duplicate any
	 * changes across projects to maintain consistency!
	 */
	public static class ChapterProperties {
		 ...
		 * @param keywords
		 *            The keywords of the chapter.
		 * @param x
		 *            The fixed x position of the chapter.
		 * @param y
		 *            The fixed y position of the chapter.
		 * @param node
		 *            The chapter's default node.
		 * @param isDefault
		 *            Is this the default chapter.
		 */
		public ChapterProperties(Color chapterColor, String description,
				String keywords, int x, int y, String node, boolean isDefault) {
	 ...
	 * 
	 * @param glyph
	 *            The VRectangle that was entered.
	...
	/**
	 * Returns if this node has a list of sub-nodes, used to determine if this
	 * is a multi-node.
	 * 
	 * @return if has a list of subnodes
	 */
